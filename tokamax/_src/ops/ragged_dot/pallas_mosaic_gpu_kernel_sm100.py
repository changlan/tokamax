...
def ragged_contracting_dim_dot_kernel_body_sm100(
  ...
  def _mask_wg():
    def _mask_loop(ki, _):
      ...
      # Wait for MMA to consume previous TMEM data before overwriting.
      ...
      kx = plgpu.broadcasted_iota(jnp.int32, (block_n, block_k), 1)
      kx = plgpu.layout_cast(kx, rhs_tile.layout)
  ...
  # Epilogue: store accumulator to output (run by MASK_WG after MMA completes).
  @pl.when(wg == _MASK_WG)
  def _epilogue():
