...
def ragged_contracting_dim_dot_kernel_sm100(
  ...
  """Pallas kernel for ragged contracting dim dot on SM100 (Blackwell).

  This kernel handles ragged dot where the contraction dimension (K) is ragged,
  meaning different groups have different K ranges. Each group produces a
  separate (M, N) output tile.

  Args:
    lhs: Left-hand side input of shape (K, M).
    rhs: Right-hand side input of shape (K, N).
    group_sizes: Group sizes of shape (G,).
    out_dtype: Output data type.
    config: Kernel configuration.
    activation: Optional activation function.

  Returns:
    Output of shape (G, M, N).
  """
  block_m = config.block_m
  block_n = config.block_n
  block_k = config.block_k
  ...
  if (lhs.dtype, rhs.dtype) != (jnp.bfloat16, jnp.bfloat16):
    raise ValueError(
        "Only the same precision bfloat16 x bfloat16 supported, got:"
  ...
  # Transpose inputs for kernel: (K, M) -> (M, K), (K, N) -> (N, K)
  x = lhs.T  # (M, K)
  w = rhs.T  # (N, K)
  ...
  # Compute group starts for K-range lookup
  ...
  def kernel(*refs, scoped):
    ...
    # Guard against empty groups to prevent deadlock
          ...
          def _mma():
            def _loop_body(ki, _):
              ...
              with jax.named_scope("load and mask rhs"):
                ...
                mask = (kx >= (group_start - k_start)) & (
                    kx < (group_end - k_start)
                )
      ...
      def _():
        ...
        with jax.named_scope("smem -> gmem"):
          plgpu.copy_smem_to_gmem(
              acc_smem, out_gmem.at[gi, slice_m, slice_n]
          )
          plgpu.wait_smem_to_gmem(0)
  ...
  return f(x, w, group_sizes, group_sizes_starts[:-1])
